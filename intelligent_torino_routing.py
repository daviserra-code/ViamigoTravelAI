...
class IntelligentTorinoRouter:
    ...
    def generate_intelligent_itinerary(self, start: str, end: str, interests: List[str] = None, duration: str = "half_day") -> List[Dict]:
        ...
        try:
            ...
            print(f"✅ Generated intelligent Torino itinerary with {len(itinerary)} stops")
            return itinerary
    ...
    def _get_torino_attractions_from_db(self, interests: List[str] = None) -> List[Dict]:
        ...
        try:
            ...
            print(f"✅ Retrieved {len(attractions)} REAL Torino attractions from database")
            return attractions
    ...
    def _build_intelligent_route(self, start: str, end: str, start_coords: List[float], 
        ...
        for i, attraction in enumerate(selected_attractions, 1):
            ...
            distance = self._calculate_distance(prev_coords, [attraction['latitude'], attraction['longitude']])
            transport = 'walking' if distance < 1.0 else ('tram' if distance < 2.0 else 'bus')
        ...
        # STEP 4: END POINT
        final_hours = int(base_time + (len(selected_attractions) + 1) * time_increment)
        final_minutes = int(((base_time + (len(selected_attractions) + 1) * time_increment) % 1) * 60)
