...
async function triggerPlanB() {
    ...
    console.log(`🧠 Triggering Piano B for ${city} with ${currentItinerary.length} stops to exclude`);
...
function renderPlanBOptions(data) {
    const alternatives = data.dynamic_alternatives || data.piano_b_alternatives || [];
    ...
    container.innerHTML = alternatives.map((alt, i) => `
        <div class="bg-gray-800 rounded-lg p-3 border border-gray-700">
            ...
            <p class="text-xs text-emerald-400">💡 ${alt.why_better || alt.ai_insight}</p>
