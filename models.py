...
class User(UserMixin, db.Model):
    ...
    travel_journals = db.relationship(
        'TravelJournal', backref='user', lazy=True, cascade='all, delete-orphan')
    smart_discoveries = db.relationship(
        'SmartDiscovery', backref='user', lazy=True, cascade='all, delete-orphan')
    plan_b_events = db.relationship(
        'PlanBEvent', backref='user', lazy=True, cascade='all, delete-orphan')
...
class TravelJournal(db.Model):
    ...
    # Pattern e preferenze dell'utente
    insights = db.Column(JSON, nullable=True)
    ...
    # Metadati AI
    # Quanto l'AI è sicura dell'analisi
    ai_confidence = db.Column(db.Float, nullable=True)
    # Suggerimenti per viaggi futuri
    generated_suggestions = db.Column(JSON, nullable=True)
...
class SmartDiscovery(db.Model):
    ...
    # 'restaurant', 'attraction', 'shop', 'event'
    discovery_type = db.Column(db.String(50), nullable=False)
    trigger_location = db.Column(
        db.String(200), nullable=False)  # Dove è stata rilevata
    suggested_place = db.Column(
        db.String(200), nullable=False)  # Cosa è stato suggerito
    ...
    relevance_score = db.Column(
        db.Float, nullable=False)  # 0-1 quanto è rilevante
    user_interests_matched = db.Column(
        JSON, nullable=True)  # Interessi che matchano
    ai_reasoning = db.Column(Text, nullable=True)  # Perché l'AI l'ha suggerita

    # Risultato
    # 'accepted', 'ignored', 'pending'
    user_action = db.Column(db.String(20), nullable=True)
...
class PlanBEvent(db.Model):
    ...
    # Scelta dell'utente
    # 'skip', 'alternative', 'reschedule'
    selected_option = db.Column(db.String(50), nullable=True)
    # Dettagli dell'alternativa scelta
    selected_alternative = db.Column(JSON, nullable=True)
    ...
    # Per migliorare future predizioni
    lessons_learned = db.Column(JSON, nullable=True)
...
class UserPreferences(db.Model):
    ...
    favorite_categories = db.Column(
        JSON, nullable=True)  # ['arte', 'cibo', 'natura']
    # 'slow', 'medium', 'fast'
    travel_pace = db.Column(db.String(20), nullable=True)
    budget_preference = db.Column(
        db.String(20), nullable=True)  # 'low', 'medium', 'high'

    # Pattern comportamentali rilevati dall'AI
    typical_duration_preference = db.Column(
        db.Float, nullable=True)  # Ore preferite
    # 0-1, tolleranza alle folle
    crowding_tolerance = db.Column(db.Float, nullable=True)
    spontaneity_score = db.Column(
        db.Float, nullable=True)  # 0-1, quanto è spontaneo
    ...
    # Metadati AI
    # Quanto l'AI è sicura del profiling
...
class AIInsight(db.Model):
    ...
    # 'pattern', 'recommendation', 'trend'
    insight_type = db.Column(db.String(50), nullable=False)
    # 'travel_behavior', 'preferences', 'efficiency'
    insight_category = db.Column(db.String(50), nullable=False)
