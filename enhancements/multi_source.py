...
class MultiSourceAggregator:
    ...
    def _get_from_wikipedia(self, place_name: str, city: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(
                "Wikipedia API not installed - pip install wikipedia-api")
        except Exception as e:
    ...
    def _merge_sources(self, sources: List[Dict]) -> Dict:
        ...
        sources_sorted = sorted(
            sources, key=lambda x: x['weight'], reverse=True)
        ...
        for source in sources_sorted[1:]:
            for key, value in source['data'].items():
                ...
                # Enrich arrays
                    ...
                    merged[key].extend(
                        [v for v in value if v not in merged[key]])
...
class DataEnrichmentPipeline:
    ...
    def enrich_place(self, place_data: Dict, city: str) -> Dict:
        ...
        enriched['visit_recommendations'] = self._generate_visit_tips(
            place_data)
        ...
        enriched['accessibility_score'] = self._estimate_accessibility(
            place_data)
...
def create_enriched_place_profile(city: str, place_name: str, category: str, db_session) -> Dict:
    ...
    logger.info(
        f"✨ Enriched profile for {place_name}: {len(enriched['sources_used'])} sources, {enriched['confidence_score']:.1f}% confidence")
