...
class CacheAnalytics:
    ...
    def __init__(self, db_session):
        ...
        self.hourly_stats = defaultdict(
            lambda: {'hits': 0, 'misses': 0, 'apify_calls': 0})
    ...
    def get_performance_stats(self) -> Dict:
        ...
        unique_cities = self.db.query(func.count(
            func.distinct(PlaceCache.city))).scalar()
...
class CostMonitor:
    ...
    def should_throttle_scraping(self, analytics: CacheAnalytics) -> bool:
        ...
        if budget_status['days_until_budget_exceeded'] and budget_status['days_until_budget_exceeded'] < 3:
            logger.warning(
                f"⚠️ Budget will be exceeded in {budget_status['days_until_budget_exceeded']} days - Throttling!")
            return True
...
class QualityValidator:
    ...
    def validate_place_data(self, place_data: Dict) -> Dict:
        ...
        if 'rating' in place_data:
            ...
            if rating < self.validation_rules['min_rating'] or rating > self.validation_rules['max_rating']:
                warnings.append(
                    f"Rating {rating} outside expected range [0-5]")
    ...
    def validate_city_coverage(self, city: str, places: List[Dict]) -> Dict:
        ...
        if validity_ratio < 0.8:
            issues.append(
                f"Low data quality: only {validity_ratio*100:.1f}% valid")
            score -= 20
