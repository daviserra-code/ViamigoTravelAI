...
class GeographicClusterer:
    ...
    def create_scraping_clusters(self, cities: List[str], max_cluster_radius: float = 150) -> List[Dict]:
        ...
        while remaining:
            ...
            cluster_coords = [self.italian_cities[c]
                              for c in cluster_cities if c in self.italian_cities]
            avg_lat = sum(c['lat']
                          for c in cluster_coords) / len(cluster_coords)
            avg_lng = sum(c['lng']
                          for c in cluster_coords) / len(cluster_coords)
        ...
        logger.info(
            f"🗺️ Created {len(clusters)} geographic clusters from {len(cities)} cities")
        return clusters
...
class BatchScrapingOptimizer:
    ...
    def optimize_scraping_order(self, cities: List[str]) -> List[Dict]:
        ...
        logger.info(
            f"⚡ Optimized {len(optimized_tasks)} scraping tasks across {len(clusters)} clusters")
...
def create_geographic_scraping_plan(cities: List[str], max_cluster_radius: float = 150) -> Dict:
    ...
    logger.info(
        f"📋 Created scraping plan: {len(cities)} cities, {len(clusters)} clusters, {len(tasks)} tasks")
    logger.info(
        f"💰 Estimated cost: ${cost_estimate['estimated_cost_usd']:.2f}, Time: {cost_estimate['estimated_time_minutes']:.1f} min")
