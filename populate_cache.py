...
ADMIN_SECRET = os.getenv('ADMIN_SECRET', 'change-this-secret-key-in-production')
        ...
        if response.status_code == 200:
            ...
            for category, result in data.get('results', {}).items():
                if result.get('cached'):
                    print(f"   {category}: {result['count']} places (already cached)")
                else:
                    print(f"   {category}: {result['count']} places (newly cached)")
...
def populate_batch(cities, delay=5):
    ...
    try:
        response = requests.post(url, json=payload, headers=headers, timeout=3600)  # 1 hour timeout
        ...
        if response.status_code == 200:
            ...
            print(f"   ({data.get('total_duration_seconds', 0) / 60:.1f} minutes)")
            print(f"{'='*60}\n")
            ...
            for city, results in data.get('results', {}).items():
                ...
                for category, result in results.items():
                    ...
                    print(f"  - {category}: {result['count']} places ({status})")
        ...
        if response.status_code == 200:
            ...
            for city, categories in sorted(data.get('cities', {}).items()):
                ...
                for category, info in categories.items():
                    ...
                    print(f"  - {category}: {info['count']} places (age: {age:.1f}h)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Populate PostgreSQL cache with Apify data')
    parser.add_argument('--city', help='Populate a single city')
    parser.add_argument('--batch', action='store_true', help='Populate all Italian cities in batch')
    parser.add_argument('--status', action='store_true', help='Check current cache status')
    parser.add_argument('--delay', type=int, default=5, help='Delay between API calls (seconds)')
    parser.add_argument('--cities-file', help='File with list of cities (one per line)')
    ...
    if args.status:
        ...
        print(f"⏱️  Estimated time: {len(ITALIAN_CITIES) * 2 * 40 / 60:.0f} minutes")
        print(f"    (2 categories × ~40s each × {len(ITALIAN_CITIES)} cities)\n")
