...
class EnhancedProactiveScrapingSystem:
        ...
        for task in optimized_tasks[:20]:  # Limit to top 20 tasks
            try:
                ...
                results['errors'].append(
                    {'city': task['city'], 'error': str(e)})

        results['completed_at'] = datetime.now()
        results['duration_seconds'] = (
            results['completed_at'] - results['started_at']).total_seconds()

        logger.info(
            f"✅ Scraping complete: {len(results['cities_scraped'])} cities, ${results['cost_usd']:.2f}")
        return results
    ...
    def _gather_priorities(self, strategy: str) -> List[Dict]:
        ...
        if strategy in ['comprehensive', 'ml_driven']:
            ...
            user_cities = self.scraping_manager.get_user_requested_cities(
                days_lookback=7)
        ...
        logger.info(
            f"📊 Gathered {len(priorities)} priorities from {len(set(p['source'] for p in priorities))} sources")
        return priorities
    ...
    def _optimize_tasks(self, priorities: List[Dict], strategy: str) -> List[Dict]:
        ...
        if strategy == 'cost_optimized':
            ...
            clusters = self.geo_clusterer.create_scraping_clusters(
                cities, max_cluster_radius=100)
            ...
            for cluster in clusters:
                for city in cluster['cities']:
                    task = next(
                        (p for p in unique_priorities if p['city'] == city), None)
                    if task:
    ...
    def _execute_scraping_task(self, task: Dict) -> bool:
        ...
        existing_data = self.scraping_manager._get_cached_places(
            city, category)
        if existing_data:
            validation = self.quality_validator.validate_city_coverage(
                city, existing_data)
    ...
    def _monitor_budget_alerts(self):
        ...
        if budget_status['alert_level'] == 'critical':
            logger.error(
                f"🚨 BUDGET CRITICAL: {budget_status['budget_used_percent']:.1f}% used!")
            # TODO: Send email/Slack alert
        elif budget_status['alert_level'] == 'warning':
            logger.warning(
                f"⚠️ Budget warning: {budget_status['budget_used_percent']:.1f}% used")
...
def register_enhanced_routes(app, scraping_system):
    ...
    def get_budget_status():
        ...
        status = scraping_system.cost_monitor.check_budget_status(
            scraping_system.analytics)
        return jsonify(status)

    @app.route('/api/admin/enhanced/geographic-clusters')
    def get_geographic_clusters():
        """Get geographic clustering plan"""
        cities = request.args.get(
            'cities', 'Roma,Milano,Firenze,Venezia,Napoli').split(',')
