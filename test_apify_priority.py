...
class ApifyPriorityTester:
    ...
    def test_repeat_request(self, attraction_name: str, times: int = 2):
        ...
        for i in range(times):
            ...
            if i == 0 and result:
                ...
                speedup = first_time / result['time'] if result['time'] > 0 else 0
                if first_source == 'apify' and result['source'] != 'apify':
                    print(f"✅ CACHING WORKS! {speedup:.1f}x faster, switched from Apify to {result['source']}")
    ...
    def run_comprehensive_test(self):
        ...
        for attraction in known_attractions:
            self.test_attraction(attraction, expected_source="comprehensive_attractions")
            time.sleep(0.5)
    ...
    def generate_report(self):
        ...
        for source, count in sorted(source_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            print(f"{emoji} {source:30s}: {count:2d} requests ({percentage:5.1f}%) - Avg: {avg_time:.2f}s")
        ...
        if apify_calls == 0:
            print("✅ EXCELLENT: No Apify calls needed! All requests served from cache/DB.")
            print("   Current priority order is optimal.")
        elif apify_calls < total * 0.1:
            print(f"✅ GOOD: Only {apify_calls} Apify calls ({(apify_calls/total)*100:.1f}%)")
            print("   Most requests served from cache. Priority order is working well.")
        elif apify_calls < total * 0.3:
            print(f"⚠️  MODERATE: {apify_calls} Apify calls ({(apify_calls/total)*100:.1f}%)")
            print("   Consider enriching database with more attractions.")
        else:
            print(f"❌ HIGH: {apify_calls} Apify calls ({(apify_calls/total)*100:.1f}%)")
