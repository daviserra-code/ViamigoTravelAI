...
class IntelligentImageClassifier:
        ...
        try:
            ...
            for term in search_terms:
                ...
                if results:
                    ...
                    confidence = self._calculate_db_confidence(
                        term, name, city)
    ...
    def _extract_search_terms(self, title: str, context: str) -> List[str]:
        ...
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on',
                      'at', 'to', 'for', 'of', 'with', 'by', 'di', 'del', 'della', 'dei'}
        ...
        for i in range(len(meaningful_words) - 2):
            terms.append(
                f"{meaningful_words[i]} {meaningful_words[i+1]} {meaningful_words[i+2]}")
    ...
    def _calculate_db_confidence(self, search_term: str, name: str, city: str) -> float:
        ...
        matches = sum(1 for word in search_words if any(
            word in nw for nw in name_words))
        if matches > 0:
    ...
    def _get_fallback_image(self, attraction: str, city: str) -> str:
        ...
        attraction_key = re.sub(
            r'[^a-z0-9]', '_', f"{attraction}_{city}".lower())
